...
export default function AiChatbot() {
  ...
  async function sendText(value) {
    ...
    try {
      ...
      if(session){const {data}=await api('/assistant/agent',{method:'POST',token:session.accessToken,body:{message:text,history}});setMessages(list=>[...list,{role:'bot',text:data.reply,toolsUsed:data.toolsUsed||[],steps:data.steps,mode:data.mode,provider:data.provider,matched:data.matched,retrievalMode:data.retrievalMode,actionToken:data.action?.token}]);setPendingAction(data.action||null)}
      else {const { data } = await api('/assistant/chat', { method: 'POST', body: { message: text, history } });setMessages(list => [...list, { role: 'bot', text: data.answer,mode:data.mode,matched:data.matched,retrievalMode:data.retrievalMode }])}
    } catch (error) {
